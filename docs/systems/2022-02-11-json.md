...
Objects one of the two complex data types in JSON. Objects help organise information and making it accessible for lookups. A JSON object starts and ends with curley brackets `{ }`. Within those brackets you then find so-called key-value pairs: `{"key": "value"}`. This is what the example API response above showed. Here is another example object:
